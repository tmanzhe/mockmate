...
export default function MicrophoneComponent() {
  ...
  const startRecording = () => {
    ...
    recognitionRef.current.onresult = (event: any) => {
        ...
        let finalTranscript = transcript; // Preserve the current transcript
        ...
        for (let i = event.resultIndex; i < event.results.length; i++) {
          ...
          if (result.isFinal) {
            ...
            interimTranscript += result[0].transcript; // Handle interim results
          }
        }
      
        // Update the state
        setTranscript(finalTranscript + interimTranscript); // Combine final and interim
    };
  ...
  const stopRecording = () => {
    if (recognitionRef.current) {
      ...
      console.log("Stopped recording " + transcript);
    }
  };
